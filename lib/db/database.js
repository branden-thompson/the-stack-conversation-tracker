 ...
 * Creates a new data/db.json file if it doesn't exist
...
async function initDb() {
  ...
  try {
    ...
    const dbFileName = process.env.NODE_ENV === 'test' ? 'db-test.json' : 'data/db.json';
...
async function migrateUserPreferences() {
  ...
  db.data.users.forEach(user => {
    ...
    // Add colorTheme preference if not present
    if (user.preferences.colorTheme === undefined) {
      user.preferences.colorTheme = 'gray'; // Default to gray color theme
      migrationCount++;
    }
  });
  
  if (migrationCount > 0) {
    console.log(`✅ Migrated ${migrationCount} users with new preferences`);
  }
}
...
export async function createUser(userData) {
  ...
  const newUser = {
    ...
    preferences: {
      ...
      colorTheme: 'gray', // Default to gray color theme as per requirements
