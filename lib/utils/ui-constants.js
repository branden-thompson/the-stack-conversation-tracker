...
export const TOOLTIP_POSITIONING = {
  // Arrow offset from right edge to center under profile picture
  profilePictureArrowOffset: '0.75rem', // 12px from right - nudged right by 4px from center
  overflowArrowOffset: '0.75rem', // Same offset for overflow indicator
};
...
export const ANIMATION_DURATION = {
  fast: 120,           // Quick transitions (reduced for responsiveness)
  normal: 180,         // Standard transitions (slightly faster)
  slow: 250,           // Slower transitions (reduced from 300ms)
  entrance: 180,       // Component entrance animations (faster)
  exit: 120,           // Component exit animations (faster)
  cardFlip: 250,       // Card flip animation duration (reduced for snappier feel)
};
...
export const ANIMATION = {
  ...
  // GPU acceleration settings for optimal performance
  gpu: {
    willChange: 'transform', // Hint browser for hardware acceleration
    backfaceVisibility: 'hidden', // Prevent flickering during transforms
    perspective: '1000px', // 3D context for hardware acceleration
  ...
  card: {
    flip: {
      ...
      // GPU optimized flip settings
      willChange: 'transform',
      backfaceVisibility: 'hidden',
      transformStyle: 'preserve-3d',
    },
    drag: {
      transition: 'transform 180ms cubic-bezier(0.4, 0, 0.2, 1)', // Faster response
      willChange: 'transform', // GPU acceleration hint
    },
    hover: {
      scale: 1.02,
      transition: 'transform 120ms ease', // Only animate transform (not 'all')
      willChange: 'transform', // GPU acceleration
    },
  },
  // Hover and interaction animations (GPU optimized)
  hover: {
    scale: {
      small: 1.05,    // Small scale for buttons/icons
      medium: 1.02,   // Medium scale for cards
      large: 1.01,    // Subtle scale for large elements
    },
    duration: ANIMATION_DURATION.fast, // Fast hover response
  ...
  zone: {
    highlight: {
      duration: 180, // Reduced from 200ms
      easing: 'ease-in-out',
    },
  },
};
...
export const DEV_LAYOUT = {
  ...
  // Auto-refresh intervals (ms) - Optimized: 2s→5s, 1s→3s (2.5x reduction in API calls)
  conversationRefreshInterval: 5000,
...
/**
 * Get dynamic APP_THEME based on color theme selection
 * This function integrates the color theme system with the base APP_THEME
 * @param {string} colorThemeId - ID of the selected color theme ('gray', 'blue', 'green', 'purple')
 * @param {string} currentTheme - Current light/dark mode ('light', 'dark')
 * @param {string} systemTheme - System theme for fallback
 * @returns {Object} Dynamic APP_THEME object
 */
export function getCurrentAppTheme(colorThemeId = 'gray', currentTheme = 'light', systemTheme = 'light') {
  ...
  try {
    ...
    if (!colorTheme) {
      console.warn(`Color theme '${colorThemeId}' not found, falling back to default APP_THEME`);
      return APP_THEME;
    }
    
    // Generate dynamic theme
    const dynamicTheme = generateAppTheme(colorTheme, currentTheme, systemTheme);
    
    return dynamicTheme;
  } catch (error) {
    console.warn('Error generating dynamic theme, falling back to default:', error);
    return APP_THEME;
  }
}