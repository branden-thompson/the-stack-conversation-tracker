...
export function transformEventsToTree(events) {
  ...
  sortedEvents.forEach(event => {
    // Handle both regular card events and flipped events (which use cardId instead of id)
    const cardId = event.payload?.id || event.payload?.cardId;
    ...
    if (event.type === 'card.created') {
      ...
      // If this is the first event for this card and no creation event exists,
      // use the first event as a pseudo-root
      if (!cardGroup.rootEvent && cardGroup.childEvents.length === 0) {
        // Create a synthetic creation event for display purposes
        cardGroup.rootEvent = {
          ...event,
          type: 'card.created',
          synthetic: true,
  ...
  const cardBranches = Array.from(cardGroups.values())
    .filter(group => group.rootEvent || group.childEvents.length > 0) // Include all cards with any events
