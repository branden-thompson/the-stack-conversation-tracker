...
// ===========================================
// CONSOLIDATED CARD CONSTANTS
// ===========================================

/**
 * Card zones (panels) with styling
 * Moved from lib/utils/constants.js for consolidation
 */
 ...
 * Enhanced from lib/utils/constants.js
 */
export const CARD_DIMENSIONS = {
  width: 320,       // baseline hint; component enforces min/max itself
  height: 200,      // default height (will be updated to use CARD_HEIGHTS if imported)
  stackOffset: 8,   // pixel offset for overlapping stacks
};
...
/**
 * Factory function to generate legacy CARD_TYPES format
 * Maintains backward compatibility while encouraging migration to new structure
 * @returns {Object} Legacy CARD_TYPES object
 */
export function generateLegacyCardTypes() {
  // Development warning (only in development mode)
  if (typeof process !== 'undefined' && process.env.NODE_ENV === 'development') {
    console.warn('DEPRECATED: CARD_TYPES is deprecated. Use TYPE_COLORS and getTypeColors() instead.');
  }

  // Generate legacy format from new TYPE_COLORS structure
  const legacyTypes = {};
  ...
  Object.keys(TYPE_COLORS).forEach(type => {
    ...
    legacyTypes[type] = {
      label,
      container: `${colors.container} ${colors.text}`,
...
/**
 * Legacy CARD_TYPES export for backward compatibility
 * @deprecated Use TYPE_COLORS and getTypeColors() instead
 */
