...
export function createGuestUser(existingUsers = [], customName = null, initialPreferences = {}) {
  ...
  try {
    ...
    console.log('Generated avatar for guest:', guestId, 'success?', !!avatarDataUrl);
  } catch (error) {
...
export const sessionGuestProvisioning = {
   ...
   */
  getOrCreateProvisionedGuest(existingUsers = []) {
    if (typeof window === 'undefined') return null;
    ...
    try {
      ...
      if (existingGuest) {
        ...
        console.log('Retrieved provisioned guest from storage:', guest.name, 'has avatar?', !!guest.profilePicture);
        
        // Verify the guest is still valid and not expired
        if (guest.sessionExpires > Date.now()) {
          ...
          if (!guest.profilePicture) {
            try {
              ...
              console.log('Generated missing avatar for existing guest:', guest.name);
      ...
      console.log('Created provisioned guest:', provisionedGuest.name, 'has avatar?', !!provisionedGuest.profilePicture);
