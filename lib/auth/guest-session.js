...
export function createGuestUser(existingUsers = [], customName = null, initialPreferences = {}) {
  ...
  // Initialize guest theme mode in localStorage (client-side only)
  if (typeof window !== 'undefined') {
    try {
      // Import only on client side to avoid SSR issues
      import('@/lib/utils/user-theme-storage').then(({ setUserThemeMode }) => {
        setUserThemeMode(guestId, 'dark');
        // Default dark mode set for guest
      });
    } catch (error) {
      console.warn('[Guest] Could not set default theme mode:', error);
    }
  }
  ...
  return {
    ...
    preferences: {
      theme: 'system', // Default theme (this is for color theme, not mode)
      themeMode: 'dark', // NEW: Default theme mode to dark for guests
      browserFingerprint,
...
export const sessionGuestProvisioning = {
  ...
  async getOrCreateProvisionedGuest(existingUsers = []) {
    ...
    try {
      ...
      // Creating new provisioned guest session
      ...
      // New guest session created successfully
      if (provisionedGuest.profilePicture) {
        // Avatar data URL generated for new guest
      }
...
export default guestSessionExports;