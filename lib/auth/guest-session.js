...
export const sessionGuestProvisioning = {
  ...
  async checkProvisionedGuestSession() {
    ...
    try {
      ...
      if (!hasActiveSession) {
        console.log('[SessionGuestProvisioning] Guest has no active session, clearing:', guest.name);
  ...
  async getOrCreateProvisionedGuest(existingUsers = []) {
    ...
    try {
      ...
      if (existingGuest) {
        ...
        console.log('[SessionGuestProvisioning] Retrieved guest from storage:', {
          name: guest.name,
          id: guest.id,
          expires: new Date(guest.sessionExpires).toLocaleTimeString(),
          isExpired: guest.sessionExpires <= Date.now()
        });
        
        // First check if guest is expired
        if (guest.sessionExpires <= Date.now()) {
          console.log('[SessionGuestProvisioning] Guest expired, clearing');
          this.clearProvisionedGuest();
        } else {
