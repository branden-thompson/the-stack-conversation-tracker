...
export function useConversationControls() {
  ...
  // OPTIMIZATION: Only run timer when there's an active conversation to prevent unnecessary re-renders
  const [tick, setTick] = useState(0);
  useEffect(() => {
    // Only start timer if there's an active conversation that needs real-time updates
    if (!activeId) {
      console.log('[ConversationControls] No active conversation - timer stopped');
      setTick(0); // Reset tick when no active conversation
      return;
    }
    
    const activeConv = (convItems || []).find(c => c.id === activeId);
    if (!activeConv || activeConv.status !== 'active') {
      console.log('[ConversationControls] No active timer needed - conversation not active');
      return; // No timer needed for paused/stopped conversations
    }
    
    console.log('[ConversationControls] Starting 1-second timer for active conversation');
    const t = setInterval(() => setTick((n) => n + 1), 1000);
    return () => {
      console.log('[ConversationControls] Stopping 1-second timer');
      clearInterval(t);
    };
  }, [activeId, convItems]);
  ...
  const runtime = useMemo(() => {
    ...
    return '00:00:00';
  }, [activeConversation, tick]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [activeConversation, patch, refreshConvos]);
