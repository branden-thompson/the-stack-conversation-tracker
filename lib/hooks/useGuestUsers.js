...
export function useGuestUsers() {
  ...
  useEffect(() => {
    const initializeSessionGuest = async () => {
      // Check if user has a persisted selection
      const persistedUserId = typeof window !== 'undefined' ? 
        window.sessionStorage.getItem('selectedUserId') : null;
      ...
      if (sessionGuest) {
        ...
        // Check for persisted user selection first
        if (persistedUserId && persistedUserId !== 'guest') {
          // User had selected a registered user - restore that selection
          // Mark that we need to restore this user
          const restoreTimer = setTimeout(() => {
            // Find the user and restore if it exists
            const userToRestore = baseUsers.users.find(u => u.id === persistedUserId);
            if (userToRestore) {
              baseUsers.switchUser(persistedUserId);
  ...
  }, [baseUsers.loading, baseUsers.users.length, baseUsers.users, baseUsers.switchUser]);
