...
export function useGuestUsers() {
  ...
  useEffect(() => {
    ...
    const initializeSessionGuest = async () => {
      ...
      if (!browserSessionId) {
        console.warn('[useGuestUsers] Browser session ID not available yet, will retry');
        // Retry after a short delay
        setTimeout(initializeSessionGuest, 500);
        return;
      }
      
      console.log('[useGuestUsers] Browser session ID for this tab:', browserSessionId);
      ...
      try {
        ...
        if (checkResponse.status === 404) {
          ...
          console.log('[useGuestUsers] Creating new browser session on server for ID:', browserSessionId);
          ...
          if (createResponse.ok) {
            ...
            console.log('[useGuestUsers] Browser session created:', browserSession.id, 
              'with provisioned guest:', browserSession.provisionedGuest?.name);
          ...
          console.log('[useGuestUsers] Restoring browser session:', browserSession.id, 
            'active user:', browserSession.activeUserId);
          ...
          if (browserSession.activeUserId) {
            if (browserSession.activeUserType === 'guest') {
              ...
              if (userToRestore) {
                ...
                console.warn('[useGuestUsers] Registered user not found, falling back to guest');
  ...
  }, [baseUsers.loading, baseUsers.users.length]);
  ...
  useEffect(() => {
    if (isGuestMode && provisionedGuest && currentGuestUser?.id !== provisionedGuest.id) {
      console.log('[useGuestUsers] Synchronizing currentGuestUser with provisionedGuest:', provisionedGuest.name);
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event) => {
      ...
      if (event.key === 'provisioned_guest_data' && event.newValue === null) {
        console.log('[useGuestUsers] Guest data cleared, reinitializing provisioned guest...');
        ...
        setTimeout(async () => {
          ...
          if (newSessionGuest) {
            ...
            console.log('[useGuestUsers] New provisioned guest created:', newSessionGuest.name);
          }
        }, 100);
      }
    };
  ...
  const handleGuestModeSwitch = useCallback(async () => {
    try {
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (browserSession.provisionedGuest) {
                ...
                console.log('[useGuestUsers] Restored provisioned guest from browser session:', guestToUse.name);
              }
            }
          } catch (error) {
      ...
      if (guestToUse) {
        ...
        // Mark the guest session as active and mark registered user sessions as inactive after delay
        setTimeout(async () => {
          try {
            ...
            console.log('[useGuestUsers] Transitioned to guest mode, updated session states');
          } catch (error) {
        ...
        console.warn('[useGuestUsers] No provisioned guest found, creating new one');
        const allUsers = [...(baseUsers.users || []), ...guestUsers];
  ...
  const handleRegisteredUserSwitch = useCallback(async (userId) => {
    ...
    // Instead of deleting guest sessions, mark them as inactive after 5s delay
    if (provisionedGuest) {
      setTimeout(async () => {
        try {
          ...
          if (response.ok) {
            console.log('[useGuestUsers] Marked guest session as inactive after switching to registered user');
          }
        } catch (error) {
  ...
  const getCurrentUser = useCallback(() => {
    ...
    // Emergency fallback - return first available user or create temp system user
    if (baseUsers.users && baseUsers.users.length > 0) {
      return baseUsers.users[0];
    }
    
    // Last resort - create a temporary system user to prevent null state
    console.warn('[useGuestUsers] No users available, creating temporary system user');
    return {
      id: 'system',
      name: 'System',
      isSystemUser: true,
  ...
  return {
    ...
    refreshProvisionedGuest: async () => {
      console.log('[useGuestUsers] Manually refreshing provisioned guest...');
