    ...
    const initializeSessionGuest = () => {
      ...
      if (sessionGuest) {
        ...
        if (persistedUserId && persistedUserId !== 'guest') {
          ...
          console.log('[useGuestUsers] Found persisted user selection:', persistedUserId);
          // Mark that we need to restore this user
          const restoreTimer = setTimeout(() => {
            console.log('[useGuestUsers] Restoring user selection to:', persistedUserId);
  ...
  const processSwitchQueue = useCallback(async () => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        console.log('[useGuestUsers] Persisted user selection:', userId);
      }
