...
export function useSSEActiveUsersOptimized(config = {}) {
  ...
  useEffect(() => {
    ...
    if (hookId) {
      ...
      console.log('[useSSEActiveUsersOptimized] Hook registered successfully:', hookId);
    } else {
      setRegistrationStatus('rejected');
      console.warn('[useSSEActiveUsersOptimized] Hook registration rejected - another hook is active');
      return;
    }
  ...
  const sseResult = useSSEIntegration({
    ...
    enabled: enabled && registrationStatus === 'registered', // CRITICAL: Only run if registered
