...
export function useUsers() {
  ...
  const switchUser = useCallback((userId) => {
    ...
    if (user) {
      ...
      // Reset auto-switch flag when manually switching
      if (userId === 'system') {
        setHasAutoSwitched(true); // Prevent auto-switching away from system
      }
    }
  }, [users]);
  ...
  // Auto-switch to a real user only once on initial load
  useEffect(() => {
    if (!hasAutoSwitched && currentUserId === 'system' && users.length > 0) {
      const regularUser = users.find(user => !user.isSystemUser);
      if (regularUser) {
        setCurrentUserId(regularUser.id);
