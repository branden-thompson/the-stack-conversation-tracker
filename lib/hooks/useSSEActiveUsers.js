...
export function useSSEActiveUsers(config = {}) {
  ...
  const allUsers = useMemo(() => {
    ...
    if (lastUsersHashRef.current !== currentHash) {
      // User hash changed, will trigger re-processing
  ...
  const fetchSessionsData = useCallback(async () => {
    try {
      if (!enabled) {
        // SSE disabled, using static data
        return;
      }
  ...
  useEffect(() => {
    ...
    if (hookId) {
      ...
      // Hook registration rejected - another hook is active
      return;
    }
  ...
  useEffect(() => {
    if (!enabled || registrationStatus !== 'registered') {
      // Hook disabled or not registered
      return;
    }
   ...
   */
  useEffect(() => {
    // Effect triggered - checking for changes
    ...
    if (lastProcessedHashRef.current === dependencyHash) {
      // Users processing skipped - no dependency changes
  ...
  }, [sessionsData, allUsers, error, loading, stableUsers]); // Added missing dependencies
