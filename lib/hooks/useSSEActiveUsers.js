/**
 * useSSEActiveUsers Hook - Phase 4 SSE Implementation with Optimization Infrastructure
 * 
 ...
 * - NEW: Integrated SSE infrastructure optimization
 */
...
import { getPollingInterval } from '@/lib/sse-infrastructure/config/environment-config.js';
...
export function useSSEActiveUsers(config = {}) {
  ...
  // Hook registry for coordination
  ...
  // Get production-safe polling interval
  const safeInterval = getPollingInterval('sessions');
  
  // DEBUG: Log the actual interval being used
  console.log('[useSSEActiveUsers] Using safe interval:', safeInterval, 'ms (should be 5000ms in dev)');
  ...
  const fetchSessionsData = useCallback(async () => {
    try {
      ...
      // FIXED: Use coordinated fetch to prevent API runaway
      const data = await coordinatedFetch('/api/sessions', {}, { enabled });
  ...
  }, [enabled, isSSESimulated, registrationStatus]);
  ...
  useEffect(() => {
    ...
    if (hookId) {
      ...
      console.log('[useSSEActiveUsers] ✅ Hook registered successfully:', hookId);
    } else {
      setRegistrationStatus('rejected');
      console.warn('[useSSEActiveUsers] ❌ Hook registration rejected - another hook is active');
      return;
    }
   ...
   * Track hook activity
   */
  const trackHookActivity = useCallback((activity, data = {}) => {
    if (hookIdRef.current) {
      registrationUtils.trackActivity(hookIdRef.current, activity, data);
    }
  }, [registrationUtils]);
  ...
  useEffect(() => {
    if (!enabled || registrationStatus !== 'registered') {
      console.log('[SSEActiveUsers] Hook disabled or not registered:', { enabled, registrationStatus });
      return;
    }
    
    // Track registration activity
    trackHookActivity('data-fetching-started');
    
    // Initial fetch
    fetchSessionsData();
    
    // FIXED: Use production-safe intervals to prevent API runaway
    console.log('[useSSEActiveUsers] Setting up interval with safeInterval:', safeInterval, 'ms');
    const fetchInterval = setInterval(() => {
      console.log('[useSSEActiveUsers] Interval tick - fetching data (interval:', safeInterval, 'ms)');
      trackHookActivity('request');
      fetchSessionsData();
    }, safeInterval);
    
    return () => {
      clearInterval(fetchInterval);
      trackHookActivity('data-fetching-stopped');
    };
  }, [enabled, registrationStatus, fetchSessionsData, safeInterval, trackHookActivity]);
  ...
  const currentReturnData = {
    ...
    // Hook coordination metadata
    _hookRegistry: {
      hookId: hookIdRef.current,
      registrationStatus,
      isActive: registrationStatus === 'registered',
      getRegistryStats: registrationUtils.getStats
    }
  };
