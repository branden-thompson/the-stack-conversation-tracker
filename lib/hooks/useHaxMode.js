    ...
    if (storage && key) {
      try {
        ...
        if (saved !== null) {
          ...
          console.log(`[useHaxMode] Loaded preference for ${currentUser?.name || 'guest'}: ${enabled}`);
        }
      } catch (error) {
  ...
  const toggleHaxMode = useCallback(() => {
    ...
    if (storage && key) {
      try {
        ...
        console.log(`[useHaxMode] Saved preference for ${currentUser?.name || 'guest'}: ${newValue}`);
      } catch (error) {
    ...
    return newValue;
  }, [isHaxMode, currentUser, getStorage, getStorageKey]);
