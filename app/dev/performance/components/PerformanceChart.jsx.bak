...
export default function PerformanceChart({ metrics, timeWindow }) {
  ...
  const chartData = useMemo(() => {
    ...
    return Object.values(buckets)
      ...
      .map(bucket => {
        ...
        // Navigation metrics
        if (bucket.navigation_timing.length > 0) {
          const navMetrics = bucket.navigation_timing;
          result.avgNavTime = navMetrics.reduce((sum, m) => sum + m.data.navigationTime, 0) / navMetrics.length;
          result.navigations = navMetrics.length;
        }
  ...
  // Chart configurations for different metric types
    ...
    switch (type) {
      ...
      case 'navigation_timing':
        return {
          ...
          dataKeys: [
            { key: 'avgNavTime', name: 'Avg Navigation Time (ms)', color: '#82ca9d' }
          ],
          yAxisLabel: 'Time (ms)'
        };
      case 'memory_usage':
      ...
      case 'performance_overview':
        return {
          ...
          dataKeys: [
            ...
            { key: 'avgNavTime', name: 'Navigation (ms)', color: '#82ca9d' },
            { key: 'pageLoadTime', name: 'Page Load (ms)', color: '#ffc658' }
          ],
          yAxisLabel: 'Time (ms)'
        };
      default:
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {[
                  { key: 'api_performance', label: 'API' },
                  { key: 'navigation_timing', label: 'Navigation' },
                  { key: 'memory_usage', label: 'Memory' },
                  { key: 'performance_overview', label: 'Overview' }
                ].map(option => (
                  <Button
                    key={option.key}
